...
class PackedDataset(Dataset):
    def __init__(
        ...
        for i in range(size):
            ...
            ### Debugging: Print the processing progress
            print(f"Processing data point {i}/{size}")

            ### Debugging: Print keys in the data point
            print(f"Keys in the data point: {list(data_point.keys())}")

            # Check attention_mask shape
            attention_mask = data_point["attention_mask"]
            print(f"Shape of attention_mask before tensor conversion: {getattr(attention_mask, 'shape', 'N/A')}")  ###

            if not isinstance(attention_mask, torch.Tensor):
                attention_mask = torch.tensor(attention_mask)

            print(f"Shape of attention_mask after tensor conversion: {attention_mask.shape}")  ###

            # Ensure that attention_mask is at least 1D
            if attention_mask.dim() == 0:
                attention_mask = attention_mask.unsqueeze(0)
            elif attention_mask.dim() == 1:
                attention_mask = attention_mask.unsqueeze(0)

            print(f"Shape of attention_mask after adjustment: {attention_mask.shape}")  ###

            # Validate the input length calculation
            try:
                input_length = torch.sum(attention_mask).item()
                print(f"Calculated input length: {input_length}")  ###
            except Exception as e:
                print(f"Error calculating input length: {e}")  ###
                raise ValueError("Invalid attention mask: Ensure it's properly formatted.")

            # Process input_ids
            input_ids = data_point["input_ids"]
            print(f"Input IDs length before slicing: {len(input_ids)}")  ###

            n_data_point = {}
            n_data_point["input_ids"] = (
                input_ids[:input_length]
                if tokenizer.padding_side == "right"
                else input_ids[-input_length:]
            )
            print(f"Processed input_ids length: {len(n_data_point['input_ids'])}")  ###

            # Process labels
            if "labels" not in data_point:
                labels = torch.tensor(n_data_point["input_ids"]).clone()
                labels[labels == tokenizer.pad_token_id] = -100  # mask pad_token
                n_data_point["labels"] = labels.tolist()
                print("Generated labels dynamically.")  ###
            else:
                n_data_point["labels"] = (
            ...
            print(f"Labels length: {len(n_data_point['labels'])}")  ###
        ...
        print(f"Pack length validation passed: pack_length={self.pack_length}, max_input_length={max_input_length}")  ###

        self.groups = pack_data_points_by_length(
            self.lengths, self.pack_length, max_packed_size
        )
        print("Packing data points into groups completed.")  ###
    ...
    def __getitem__(self, i) -> Dict[str, torch.Tensor]:
        ...
        packed_data = pack_data_points_FA(
            group_data_points, self.tokenizer, self.max_input_length
        )

        # Debug: Print shapes of all tensors in packed_data
        for key, value in packed_data.items():
            if isinstance(value, torch.Tensor):
                print(f"Dataset __getitem__: {key} shape = {value.shape}")
            else:
                print(f"Dataset __getitem__: {key} type = {type(value)}")

        # Ensure tensors are at least 1-dimensional
        for key, value in packed_data.items():
            if isinstance(value, torch.Tensor) and value.dim() == 0:
                packed_data[key] = value.unsqueeze(0)
